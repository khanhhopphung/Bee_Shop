...
import { Table, Button, Modal, Form, Input, message, Select, Switch, Upload, Space } from 'antd';
import { DeleteOutlined, EditOutlined, PlusOutlined, UploadOutlined, EyeOutlined, SearchOutlined } from '@ant-design/icons';
...
const ProductVariants: React.FC = () => {
  ...
  const columns: ColumnsType<ProductVariant> = [
    ...
    {
      title: <span style={{ fontSize: '18px', fontWeight: 'bold' }}>Tồn kho</span>,
      dataIndex: 'stock',
      key: 'stock',
      render: (text: string) => <span style={{ fontSize: '16px' }}>{text}</span>,
      align: 'right',
  ...
  return (
    <div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      <Modal
        open={isModalVisible}
        title={<span style={{ fontSize: '20px', fontWeight: 'bold' }}>{currentVariant ? 'Chỉnh sửa biến thể' : 'Thêm biến thể'}</span>}
