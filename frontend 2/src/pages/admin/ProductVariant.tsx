...
const ProductVariants: React.FC = () => {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      if (currentVariant) {
        await axios.put(
          `http://127.0.0.1:8000/api/product-variants/${currentVariant.id}`,
          formData,
          {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "product_id",
      render: (id: number) => products.find((p) => p.id === id)?.name || "N/A",
    },
    {
      title: "Kích thước",
      dataIndex: "size_id",
    ...
    },
    {
      ...
      render: (record: ProductVariant) => (
        <img
          src={
            record.image_url
              ? `http://127.0.0.1:8000/storage/${record.image_url}`
              : ""
          }
          alt="Hình sản phẩm"
